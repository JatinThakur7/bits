...
Future<List<Country>> getCountries(BuildContext context) async {
  String rawData = await DefaultAssetBundle.of(context)
      .loadString('packages/country_picker/assets/raw/country_codes.json');
  if (rawData == null) {
    return [];
  }
...
Future<Country> getDefaultCountry(BuildContext context) async {
  ...
  try {
    ...
    return list.firstWhere((e) => e.countryCode == countryCode);
  } catch (e) {
    print(e);
  }
  return await getCountryByCountryCode(context, 'IN');
}
